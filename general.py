...
sc = StandardScaler()
dataset['REGION_RATING_CLIENT_W_CITY'] = sc.fit_transform(dataset['REGION_RATING_CLIENT_W_CITY'].values.reshape(-1, 1))
dataset = dataset.drop(['SK_ID_CURR'],axis=1)

#Splitting into X and Y
X = dataset.iloc[:, 0:15000].values
y = dataset.iloc[:, 15000:30000].values

# Number of data points in the minority class
number_records_fraud = len(dataset[dataset.TARGET == 1])
fraud_indices = np.array(dataset[dataset.TARGET == 1].index)

# Picking the indices of the normal classes
normal_indices = dataset[dataset.TARGET == 0].index
...
X_undersampled = undersampled_data.iloc[:, 0:15000].values
y_undersampled = undersampled_data.iloc[:, 15000:].values
